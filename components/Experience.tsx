  ...
  return (
    <section className="py-20 bg-[#0b0f17]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-4xl font-bold font-display mb-16 tracking-tight">
          Experience
        </h2>
        <div className="relative pl-8 border-l-2 border-[#314368] space-y-16">
          {experience.map((item) => (
            <div key={item.id} className="relative group">
              <div
                className={`absolute -left-[42px] top-1.5 w-6 h-6 rounded-full border-4 border-background-dark flex items-center justify-center transition-colors duration-300 ${
                  item.current
                    ? 'bg-primary'
                    : 'bg-[#314368] group-hover:bg-primary'
                }`}
              >
              ...
              <h4 className="text-xl font-medium text-slate-300 mb-6">
                {item.company}
              </h4>
              ...
              <ul className="space-y-4">
                {item.description.map((point, idx) => (
                  <li
                    key={idx}
                    className="flex items-start gap-4 text-slate-400 leading-relaxed max-w-3xl"
                  >
